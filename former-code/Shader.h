...
public:
    ...
    Shader(std::string vertexPath, std::string fragmentPath); // 构造着色器: vertex + fragment
    Shader(std::string vertexPath, std::string fragmentPath, std::string geometryPath); // 构造着色器 + geometry
    void use(); // 激活着色器函数
...
Shader::Shader(std::string vertexPath, std::string fragmentPath, std::string geometryPath){ // 构造着色器 + geometry
    // 1.读取着色器source code
    ...
    try{
        ...
        vertexFile.open(std::string("../") + vertexPath);
        fragmentFile.open(std::string("../") + fragmentPath);
        geometryFile.open(std::string("../") + geometryPath);
